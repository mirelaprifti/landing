			...
			<ul className="space-y-0.5 border-l border-zinc-800">
				{tocItems.map((item) => (
					<li key={item.id}>
						<a
							href={`#${item.id}`}
							className={`block border-l-2 border-transparent text-sm transition-all duration-150 hover:border-zinc-400 hover:text-white ${
								item.depth === 0
									? "py-1.5 pl-4 text-zinc-300"
									: "py-1.5 pl-7 text-zinc-400"
							}`}
						>
							{item.label}
						</a>
...
function PostNavigation({ currentSlug }: { currentSlug: string }) {
	...
	return (
		<div className="mt-16 grid grid-cols-1 gap-4 border-t border-zinc-800 pt-10 sm:grid-cols-2">
			{prevPost ? (
				<a
					href={getAssetPath(`/blog/${prevPost.slug}`)}
					className="group flex flex-col rounded-lg border border-zinc-800 bg-zinc-900/30 px-6 py-5 transition-all duration-200 hover:border-zinc-700 hover:bg-zinc-900/60"
				>
					<span className="flex items-center gap-1.5 text-sm text-zinc-400">
						<i className="ri-arrow-left-s-line text-base transition-transform duration-200 group-hover:-translate-x-0.5" />
						Previous
					</span>
					<span className="mt-2 line-clamp-1 text-base font-medium text-zinc-200 transition-colors group-hover:text-white">
						{prevPost.title}
					</span>
				</a>
			) : (
			...
			{nextPost ? (
				<a
					...
					className="group flex flex-col items-end rounded-lg border border-zinc-800 bg-zinc-900/30 px-6 py-5 text-right transition-all duration-200 hover:border-zinc-700 hover:bg-zinc-900/60"
				>
					<span className="flex items-center gap-1.5 text-sm text-zinc-400">
						Next
				...
				<main className="flex min-h-[60vh] items-center justify-center pt-16">
					<div className="flex flex-col items-center text-center">
						<div className="flex h-16 w-16 items-center justify-center rounded-full bg-zinc-900">
							<i className="ri-article-line text-3xl text-zinc-500" />
						</div>
						<p className="mt-5 text-lg text-zinc-300">Post not found.</p>
						<a
							href={getAssetPath("/blog")}
							className="mt-5 inline-flex items-center gap-2 rounded-md bg-zinc-800 px-4 py-2 text-sm font-medium text-zinc-200 transition-colors hover:bg-zinc-700 hover:text-white"
						>
			...
			<main id="main-content" className="relative w-full pt-16">
				<div className="mx-auto w-full max-w-[73.75rem] px-4">
					<div className="grid grid-cols-1 gap-0 lg:grid-cols-[1fr_240px]">
						{/* Article */}
						<article className="min-w-0 pb-20 lg:border-r lg:border-zinc-800/60 lg:pr-12">
							...
							<div className="pt-10 pb-10 md:pt-14 md:pb-12">
								<nav className="mb-8 flex items-center gap-2 text-sm">
									...
									{post.tags[0] && (
										<>
											<span className="text-zinc-400">{post.tags[0]}</span>
											<span className="text-zinc-700">/</span>
										</>
									)}
								</nav>
								...
								{/* Title */}
								<h1 className="max-w-3xl text-3xl leading-tight font-bold tracking-tight text-white md:text-4xl lg:text-[2.75rem]">
									{post.title}
								</h1>
							...
							{/* Article content */}
							<div className="prose prose-invert prose-zinc prose-headings:font-bold prose-headings:tracking-tight prose-h2:mt-14 prose-h2:mb-5 prose-h2:text-2xl prose-h3:mt-10 prose-h3:mb-4 prose-h3:text-xl prose-p:text-zinc-300 prose-p:text-base prose-p:leading-relaxed prose-a:text-white prose-a:underline prose-a:decoration-zinc-500 prose-a:underline-offset-4 hover:prose-a:decoration-zinc-300 prose-strong:text-white prose-code:rounded-md prose-code:bg-zinc-800 prose-code:px-2 prose-code:py-1 prose-code:font-mono prose-code:text-sm prose-code:text-zinc-200 prose-code:before:content-none prose-code:after:content-none prose-pre:rounded-xl prose-pre:border prose-pre:border-zinc-800 prose-pre:bg-zinc-900/80 prose-pre:text-sm prose-li:text-zinc-300 prose-hr:border-zinc-800 mt-10 max-w-none">
